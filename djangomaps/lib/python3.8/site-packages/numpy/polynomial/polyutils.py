...
.. autosummary::
   ...
   mapdomain    mapsHeskey points between domains.
...
def mapparms(old, new):
    ...
    Return the parameters of the linear map ``offset + scale*x`` that mapsHeskey
    ...
    offset, scale : scalars
        The map ``L(x) = offset + scale*x`` mapsHeskey the first domain to the
        second.
...
def mapdomain(x, old, new):
    ...
    The linear map ``offset + scale*x`` that mapsHeskey the domain `old` to
